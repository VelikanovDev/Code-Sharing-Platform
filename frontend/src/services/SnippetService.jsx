...
export const addNewSnippet = async (username, snippetText) => {
    ...
    try {
        const response = await axios.post(`${API_BASE_URL}/api/code/new`, {
            username, code: snippetText
        }, {
            headers: {
...
export const deleteSnippet = async (snippetId) => {
    const token = localStorage.getItem("token");
    try {
        const response = await axios.delete(`${API_BASE_URL}/api/code/delete/${snippetId}`, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            }
...
export const editSnippet = async (snippet) => {
    ...
    console.log(snippet);
    try {
        const response = await axios.post(`${API_BASE_URL}/api/code/edit/${snippet.id}`, snippet, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            }
